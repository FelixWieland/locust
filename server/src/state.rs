...
impl GlobalState {
    ...
    pub fn add_session(self: Arc<Self>, session: Session) -> Arc<Session> {
    ...
    pub async fn subscribe_to_node(
        ...
        receiver_id: &Uuid,
        node_id: &Uuid,
    ) -> Option<Receiver<Option<Value<Any>>>> {
        match self.runtime.get_node(node_id) {
            Some(n) => Some(n.subscribe(receiver_id).await),
            None => None,
        }
    }

    pub async fn unsubscribe_from_node(&self, receiver_id: &Uuid, node_id: &Uuid) {
        let n = self.runtime.get_node(node_id);
        if let Some(n) = n {
            n.unsubscribe(receiver_id).await
        }
    }
}
